...
export function HiddenContent({ isPrivateKey, value }: HiddenContentProps) {
  const [isVisible, setIsVisible] = useState(isPrivateKey)
  ...
  const handleCopyToClipboard = () => {
    copyToClipboard(
      value,
      `You copied the ${isPrivateKey ? 'private' : 'public'} key successfully.`,
      `Failed to copy the ${isPrivateKey ? 'private' : 'public'} key.`,
      `You successfully copied the ${isPrivateKey ? 'private' : 'public'} key.`
    )
    setCopied(true)
