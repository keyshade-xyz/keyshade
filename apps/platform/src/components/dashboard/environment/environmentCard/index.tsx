...
export default function EnvironmentCard({
  ...
  const isAuthorizedToEditEnvironment = environment.entitlements.canWrite
  const isAuthorizedToDeleteEnvironment = environment.entitlements.canDelete
    ...
    <ContextMenu key={environment.id}>
        ...
        >
          <div className="flex flex-col gap-y-2 px-6 py-4">
            <div className="flex w-full flex-row flex-wrap items-center justify-between gap-4">
              <div className="truncate text-2xl">{environment.name}</div>
              <CopyToClipboard text={environment.slug} />
            </div>
            {environment.description ? (
              <div className="truncate text-sm font-semibold text-white/50">
                {environment.description}
              </div>
            ) : null}
      ...
      <ContextMenuContent className="flex w-[15.938rem] flex-col items-center justify-center rounded-lg bg-[#3F3F46]">
        <ContextMenuItem
          ...
          disabled={!isAuthorizedToEditEnvironment}
        ...
        <ContextMenuItem
          className="h-[33%] w-[15.938rem] text-xs font-semibold tracking-wide"
          disabled={!isAuthorizedToDeleteEnvironment}
